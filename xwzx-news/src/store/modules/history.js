...
export const useHistoryStore = defineStore('history', {
  ...
  actions: {
    ...
    removeHistory(id) {
      ...
      this.history = this.history.filter(item => item.id !== targetId);
      this.saveHistory();
    },
    ...
    async getHistoryListApi() {
      ...
      try {
        ...
        if (response.data.code === 200) {
          ...
          console.log('获取浏览历史API：成功获取', historyList.length, '条记录');
          
          // 只有当API返回数据时才覆盖本地，否则保留本地数据
          if (historyList.length > 0) {
            this.history = historyList;
            this.saveHistory();
          } else {
            // API返回空，但可能本地有数据（未同步到服务器的记录）
            this.loadHistory();
          }
