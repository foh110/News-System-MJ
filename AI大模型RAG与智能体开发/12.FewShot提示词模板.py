...
few_shot_template = FewShotPromptTemplate(
    ...
    suffix="基于前面的示例，告知我{input_word}的反义词是什么?",      #示例之后的提示词
    input_variables=["input_word"]    #声明在前缀或后缀中所需要注入的变量名
)
prompt_text = few_shot_template.invoke(input={"input_word":"善良"}).to_string()
print(prompt_text)

moedl=Tongyi(model="qwen-max")
print(moedl.invoke(input = prompt_text))